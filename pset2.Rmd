...
$$
L(\lambda) = \prod_{i=1}^{n}P(X_i = x_i) = \prod_{i=1}^{n} \left(\frac{e^{-\lambda}\lambda^{x_i}{x_i!}\right)
$$
...
\ell(\lambda)
&= \log L(\lambda) \\
&= \log\left( \prod_{i=1}^n \frac{e^{-\lambda}\lambda^{x_i}}{x_i!} \right) \\
&= \sum_{i=1}^n \log\left( \frac{e^{-\lambda}\lambda^{x_i}}{x_i!} \right) \\
&= \sum_{i=1}^n ( x_i \log \lambda -\lambda - \log(x_i!) ) \\
&= \log \lambda\sum_{i=1}^nx_i - n \lambda - \sum_{i=1}^n\log(x_i!)
\end{aligned}
$$
...
Where $\ell(\lambda) = \log \lambda\sum_{i=1}^nx_i - n \lambda - \sum_{i=1}^n\log(x_i!)$
...
= 0 \\[8pt]
\frac{1}{\lambda}\sum_{i=1}^n x_i = n \\[6pt]
\frac{1}{\lambda} = \frac{n}{\sum_{i=1}^n x_i} \\[6pt]
\lambda = \frac{\sum_{i=1}^n x_i}{n}.
\end{aligned}
$$

Therefore, the maximum likelihood estimator is $\hat{\lambda} = \frac{\sum_{i=1}^n x_i}{n}$.
...
The $\hat{\lambda}$ is equal to the sample mean of the distribution; in this example therefore $\hat{\lambda}$ = 1.4
...
c.  Alternatively, we may consider using the priors below: \begin{align*}
    ...
    \end{align*} Please plot, on a common $\theta \in [0,1]$ axis, the posterior densities for the four priors (the baseline prior in part (a) and the alternative priors above).

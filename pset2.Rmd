...
output: pdf_document
author: "Group: Carl Harper, Eduardo Monte Jorge Hey Martins, Sofia Breganni" 
date: "2025-11-11"
header-includes:
  - \usepackage{comment}
...
\Pr(\bar{X}_{100} \le 1.7)
  = \Phi\!\left(\frac{1.7 - 2}{\sqrt{2/100}}\right)
  = 0.0169.
$$
...
To reduce this probability below $1\%$, we need at least $n_{\text{required}} = `r n_required`$ observations.
...
&= \sum_{t=1}^n (-\lambda + x_t \log \lambda - \log(x_t!) ) \\
&= - n \lambda + \log \lambda\sum_{t=1}^nx_t - \sum_{t=1}^n\log(x_t!)
\end{aligned}
$$
...
&= - 5 \lambda + \log \lambda(1+3+1+2+0) - (\log(1) + \log(6) + \log(1) + \log(2) + \log(1)) \\
&= - 5 \lambda + 7\log \lambda - 2.48
\end{aligned}
$$
...
Where $\ell(\lambda) = - n \lambda + \log \lambda\sum_{t=1}^nx_t - \sum_{t=1}^n\log(x_t!)$
...
If prior distribution is a beta distribution, so is the posterior \$\$ \begin{align*}
p(\theta|y,n)
...
\\
p(\theta|44,274)
&\propto \theta^{2+44-1}(1-\theta)^{10+274-44-1} \\
&\propto \theta^{45}(1-\theta)^{239} \\
   
\end{align*} \$\$

this is the kernel of the beta distribution with updated parameters $$
\begin{align*}
 \alpha&=\alpha+y, \ \beta=\beta+n-y \\ 
  \alpha&=2+44,  \ \beta=10+274-44 \\
   \alpha&=46, \  \beta=240 \\
 \\
 &=Beta(\theta; 46, 240) \\
\end{align*}   
...
theta <- seq(0.001, 0.999, length.out = 1000) #to make sure the jeffrey's type weak prior does not produce errors
...
From comparing the four priors, prior strength (the sum α + β) has the most substantial impact on the posterior. The three weak priors—Beta(2,10), Beta(1,1), and Beta(0.5,0.5)—all have small α + β values (12, 2, and 1 respectively) and yield nearly identical posteriors centered around the data proportion of 44/274 ≈ 0.16, with posterior means approximately around the posterior mean/mode of 0.16. This demonstrates that when priors are weak, the data dominates the inference. Ultimately, the less assumptions you go into analysis with, the more the data speaks for itself. In contrast, the strong prior Beta(100, 2) with α + β = 102 produces a much different posterior with mean ≈ 0.38, which shifts the distribution considerably to the right.

The three weak priors are most defensible in this context. They align with the objective/reference prior approach (Jeffreys prior) and let the data speak for itself, consistently estimating algae presence at 12-20% of freshwater sites. This suggests algae is present but not dominant. The baseline Beta(2,10) prior is particularly reasonable as its prior mean (0.17) happens to align closely with the observed data proportion, though this prior is still weak enough not to dominate the inference. The Beta(100, 2) prior would only be defensible if there were strong external evidence suggesting much higher algae presence, such as one that reflects its much higher prior mean.

...
```
Using the Central Limit Theorem, we approximate the sampling distribution
of the sample mean as
\[
\bar{X}_{100} \approx \mathcal{N}\!\left(2,\; \frac{2}{100}\right).
\]

The probability that the observed mean is \(1.7\) or lower, even though the
true mean is \(2\), is
\[
...
This corresponds to about \(1.7\%\), meaning there is a 1.7\% chance of falsely
concluding that the mean is below \(1.7\).
...
# Interpretation output:
```
To reduce this probability below \(1\%\), we need at least
\(n_{\text{required}} = `r n_required`\)
observations.
